                            ...
                            onClick={async () => {
                                try {
                                    const response = await fetch("/api/paymentlink", {
                                        ...
                                        body: JSON.stringify({ amount: Number(data.selling_price) }),
                                    });

                                    const result = await response.json();
                                    window.open(result.data.url, '_blank');
                                    let order_id = "/OrderTracking?orderId="+ result.data.mch_order_no
                                    router.push(order_id);
                                } catch (err) {
                                    console.error("Error calling /api/initiate:", err);
